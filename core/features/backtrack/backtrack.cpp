...
void c_backtrack::update() noexcept {
	...
	if (!config_system.item.legitbot.backtrack || !local_player || !local_player->is_alive()) {
		if (!records->empty())
			records->clear();
	...
	for (int i = 1; i <= interfaces::globals->max_clients; i++) {
		...
		while (records[i].size() > 3 && records[i].size() > static_cast<size_t>(time_to_ticks(static_cast<float>(config_system.item.legitbot.backtrack_ms) / 1000.f)))
			records[i].pop_back();
